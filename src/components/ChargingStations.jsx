...
export default function ChargingStations({ directions, map, stationType }) {
  ...
  const getStations = async () => {
    ...
    if (cache) {
      ...
      const stations = await fetch(stationsEndpoint, { method: "GET" }).then(
      ...
      const newStations = stations.reduce((res, s) => {
        for (let a = 0; a < pathPoints.length; a++) {
          const pathPointCoord = pathPoints[a],
            stationCoord = new window.google.maps.LatLng(
              s.latitude,
              s.longitude
            ),
  ...
  const filterStationsByType = (type) => {
    if (type !== "ALL") {
      ...
      const filteredStations = chargingStations.filter((s) =>
        s.socket_type.includes(stationType)
      );
  ...
  }, [stationType]); // eslint-disable-line react-hooks/exhaustive-deps

  return <Markers map={map} stations={filteredStations} />;
}
